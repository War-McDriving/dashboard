...
def create_heatmap(file_path):
    ...
    try:
        data = pd.read_csv(file_path, usecols=['MAC', 'SSID', 'AuthMode', 'FirstSeen', 'Channel', 'RSSI', 'CurrentLatitude', 'CurrentLongitude', 'AltitudeMeters', 'AccuracyMeters', 'Type'], low_memory=False).dropna()
    except KeyError:
        raise ValueError("De vereiste kolommen ontbreken in het bestand.")

    # Filter alleen numerieke waarden met een enkele selectie
    data = data[(data['CurrentLatitude'].apply(pd.api.types.is_number)) &
    ...
    grouped_data = data.loc[data.groupby('SSID')['RSSI'].idxmax()].reset_index(drop=True)
    ...
    if not grouped_data.empty:
        ...
        wifi_map = folium.Map(location=map_center, zoom_start=15, width='100%', height='400px')

        # Voeg HeatMap toe
        heat_data = grouped_data[['CurrentLatitude', 'CurrentLongitude', 'RSSI']].values.tolist()
